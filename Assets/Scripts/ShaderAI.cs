...
public class ShaderAI : MonoBehaviour
{
    ...
    [Header("?? 감지 범위")]
    [Range(0, 50)] public float warningRadius = 20f;   // 1차: 경고 & 추격 포기 거리
    ...
    void Update()
    {
        ...
        if (isAggroed)
        {
            // ★ 수정됨: 화난 상태여도 거리가 멀어지면 진정함
            // (여기서는 warningRadius를 도망 성공 기준으로 잡았습니다)
            ...
            else
            {
                // 여전히 범위 내에 있으면 계속 추격
                ChasePlayer();
                // 추격 중일 때는 아래의 배회 로직을 실행하지 않고 리턴
        ...
        else if (distance <= warningRadius)
        {
            ...
            {
                Debug.Log("?? [Shader] 주변 공기가 차가워진다... (Warning)");
